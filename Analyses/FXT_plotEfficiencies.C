...
void FXT_plotEfficiencies(){
  gROOT->Reset();
  c= new TCanvas();
  //const Float_t pi_m=0.494;
  ROOT::EnableImplicitMT(); //enable multi threading - application must be MT safe ..your job
  TH1::SetDefaultSumw2(); //!!!
  ...
  TPlotDefinitions PlotDefs= FXT_Efficiency_plots;
...
//  TFile *f=new TFile("eff_FXT_2020_5p75.root","recreate");
  TFile *f=new TFile("eff_FXT_2020_5p75_Zleft_VrOuter.root","recreate");
  DrawEffs(Res_Plots);
  f->Write();
  f->Flush();
  //f->Close();
