...
std::vector<int> StKFParticleInterface::GetPID(double m2, double p, int q, double dEdXPull[7], bool isTofm2, const int trackId)
{
  vector<int> ToFPDG;
  if(isTofm2)
...
//   if(!IsGoodPV(primaryVertex)) return 0;
  ...
  for (Int_t iTrack = 0; iTrack < nGlobalTracks; iTrack++) 
  {
    ...
    vector<int> totalPDG = GetPID(m2tof, track.GetP(), q, dEdXPull, isTofm2, index);
